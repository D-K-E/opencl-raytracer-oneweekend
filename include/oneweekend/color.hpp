...
inline int cast_color(float x) {
  return int(clamp(x, 0.0f, 0.999) * 256);
}
...
void save_to_ppm(const char *ppm_path, int imwidth,
                 int imheight, cl_float4 *cpu_out) {
  std::ofstream ppm;
  ppm.open(ppm_path);
  ...
  */
  for (int j = imheight - 1; j >= 0; j--) {
    for (int i = 0; i < imwidth; i++) {
      cl_int3 color = cast_color(cpu_out[j * imwidth + i]);
      int ir = color.s[0];
      int ig = color.s[1];
      int ib = color.s[2];
      ppm << ir << " " << ig << " " << ib << std::endl;
