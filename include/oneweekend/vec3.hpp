...
public:
  ...
  Vec3 &operator/=(const cl_float t) { return *this *= 1.0f / t; }

  cl_float length() const { return std::sqrt(length_squared()); }
...
inline Vec3 operator*(const Vec3 &v, cl_float t) { return t * v; }

inline Vec3 operator/(Vec3 v, cl_float t) { return (1 / t) * v; }

inline cl_float dot(const Vec3 &u, const Vec3 &v) {
  return u.e.s[0] * v.e.s[0] + u.e.s[1] * v.e.s[1] + u.e.s[2] * v.e.s[2];
}
